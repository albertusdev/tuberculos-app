...
class ApotekerInputDailyAlarmScreen extends StatelessWidget {
  _handleSubmit(BuildContext context, Store<AppState> store) async {
    try {
      ...
      if (state.quantity == null) {
        throw "Butir obat tidak boleh kosong";
      }
  ...
  void _handleInputQuantity(BuildContext context, Store<AppState> store) async {
    String quantityString =
        await Navigator.of(context).push<String>(new MaterialPageRoute(
              builder: (_) => new NumberInputScreen(
                  title: "Butir obat",
                  hintText: "Masukkan butir obat untuk sekali minum"),
            ));
    if (quantityString != null && quantityString.isNotEmpty) {
      try {
        int quantity = int.parse(quantityString);
        store.dispatch(new ActionInputAlarmSetQuantity(quantity));
      } catch (e) {
        Scaffold.of(context).showSnackBar(new SnackBar(
            content: new Text("Seluruh digit harus merupakan angka.")));
      }
                                  ...
                                  child: new Column(
                                    children: <Widget>[
                                      ...
                                      new OutlineButton(
                                        ...
                                        onPressed: () => _handleInputTimeOfDay(
                                            context, store),
                                      )
                                    ],
                                    mainAxisSize: MainAxisSize.min,
                                    ...
                                    child: new Column(
                                      children: <Widget>[
                                        ...
                                        new OutlineButton(
                                          ...
                                          onPressed: () => _handleInputQuantity(
                                              context, store),
                                        ),
                                      ],
                                      mainAxisSize: MainAxisSize.min,
